...
　FD又はFDLコマンドでLOAD実行すればSD-CARD対応版SWORDとなりますが、RESETボタンを押すと動作がおかしくなるので注意が必要です。
...
　RESETボタンを押すと動作がおかしくなるので注意が必要です。RESETボタンを押しても動作がおかしくならないようにするには、S-OS SWORDと同様にSD対応MONITORを含めてMZTファイルを作成してください。
