...
### S-OS SWORD TAPE版
MZ-80B版、MZ-2000、MZ-2200版はSDからBOOTしたランチャー用MONITORからFD又はFDLコマンドでLOAD実行すればそのままSD対応となりますが、SDアクセスルーチンを保護するためにS-OSで使用できるメモリの上限を示す#MEMAXR値を1300Hから始まるSWORDのMZTファイルをバイナリエディタで以下のように修正します。
