...
　インタフェース基板は、安価に発注するために10cmx10cmで設計していますのでMZ-80B、MZ-2000の拡張カードとしては寸足らずです。右側をガイドスロットに沿って差し込めるように設計しています。
...
## アプリケーションからSD-CARDとのLOAD、SAVEするため対応方法
### BASIC
#### MZ-80B SB-5520
...
　27C512を使えば二つのバイナリファイルを切り替えて使うような使い方ができますので一つをSD対応MONITOR、一つをROM起動SD対応BASICとすればアプリケーションランチャーとスイッチオンBASICを切り替えて使うことができます。

　BASICをROMに焼くには、各BASICにSD対応の修正を加えたうえでMZTファイルのヘッダを削除し、以下の修正を加えてROMに焼きます。
|ADDRESS|修正前|修正後|
| ------------ | ------------ | ------------ |
|0000|C3 3B 00|00 18 38|
