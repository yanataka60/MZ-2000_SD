...
　FD又はFDLコマンドでLOAD実行すればSD-CARD対応版SWORDとなりますが、RESETボタンを押すと動作がおかしくなりますので注意が必要です。

　RESETボタンを押しても動作がおかしくならないようにするには、SD対応としたMONITOR(SB-1520、MZ-1Z001M)の0000h～129Fhを0000h～129Fhに配置、S-OS SWORD本体を1300h～に配置したうえで00AEhからの3ByteをC3 FD 1Fと修正してヘッダを付加して実行アドレス0000hのMZTファイルとして保存します。

　このMZTファイルをFD又はFDLコマンドでLOAD実行すればRESETしてもS-OS SWORDのコールドスタートに戻ります。
...
　RESETボタンを押すと動作がおかしくなりますので注意が必要です。RESETボタンを押しても動作がおかしくならないようにするには、S-OS SWORDと同様にSD対応MONITORを含めてMZTファイルを作成してください。
