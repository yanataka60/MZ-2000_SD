...
;スタックポインタをFFFFHに設定
		...
		LD		DE,(FSIZE2)  ;SADRS+FSIZE
		LD		HL,(SADRS2)
		DEC		HL
		ADD		HL,DE
		LD		DE,0801H    ;SADRS+FSIZE-1+801H < 10000H?
		ADD		HL,DE
		JR		NC,SDLD3    ;終了アドレスがF7FFH未満なら通常のデータ受信

		LD		HL,(SADRS2) ;SASRD+ED60H > FFFFH
		LD		DE,0ED60H
		ADD		HL,DE
		JR		C,SDLD2     ;スタートアドレスが12A0H以上なら特別データ受信へ

SDLD3:	CALL	DBRCV      ;データ受信
...
SDLD2:	LD		HL,FSIZE2  ;データ受信ルーチンだけを1200H～に転送して制御を移す。
		LD		DE,1200H
		LD		BC,004FH
		LDIR
		JP		1206H
...
HDRC1:	CALL	RCVBYTE    ;ファイルネーム受信
		...
		CALL	MSGPR
		CALL	LETLN
		LD		HL,SADRS2  ;SADRS取得
		CALL	RCVBYTE
		LD		(HL),A
		INC		HL
		CALL	RCVBYTE
		LD		(HL),A
		LD		HL,FSIZE2   ;FSIZE取得
		CALL	RCVBYTE
		LD		(HL),A
		INC		HL
		CALL	RCVBYTE
		LD		(HL),A
		LD		HL,EXEAD2   ;EXEAD取得
		CALL	RCVBYTE
		LD		(HL),A
		INC		HL
		CALL	RCVBYTE
...
ENT6:
FNAME2	DB		00H,00H,00H,00H,00H,00H,00H,00H,00H,00H,00H,00H,00H,00H,00H,00H,00H
EADRS2:
FSIZE2	DB		00H,00H
SADRS2	DB		00H,00H
EXEAD2	DB		00H,00H
